...
public class CreditClassForEnrollCourseAdapter extends ArrayAdapter implements Filterable {
    Context context;
    int resource;
    private String searchQuery;
    ArrayList<EnrollCourseItem> data_org;
    ArrayList<EnrollCourseItem> data_view;
    List<EnrollCourseItem> listChoseCourse;

    public CreditClassForEnrollCourseAdapter(Context context, int resource) {
    ...
    public void setListChoseCourse(List<EnrollCourseItem> listChoseCourse) {
        this.listChoseCourse = listChoseCourse;
    }
    ...
    public Filter getFilter() {
        return new Filter() {
            ...
            protected void publishResults(CharSequence constraint, FilterResults results) {
                ...
                CreditClassForEnrollCourseAdapter.super.notifyDataSetChanged();
            }
        };
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(enrollCourseItem.getSoLuongCon()==0||enrollCourseItem.getStatusEnroll()== StatusEnroll.DALUU
                || listChoseCourse.stream().filter(item -> !item.getMaLopTc().equals(enrollCourseItem.getMaLopTc()))
                    .map(EnrollCourseItem::getMaMh)
                    .anyMatch(item -> item.equals(enrollCourseItem.getMaMh()))){
            viewHolder.cbDangKy.setEnabled(false);
        }
        else{
            viewHolder.cbDangKy.setEnabled(true);
        }
        ...
        viewHolder.cbDangKy.setChecked(enrollCourseItem.getStatusEnroll() != StatusEnroll.CHUACHON);
        ...
        viewHolder.cbDangKy.setOnClickListener(view -> {
            if(enrollCourseItem.getStatusEnroll()==StatusEnroll.CHUACHON){
                Optional<EnrollCourseItem> courseItem = listChoseCourse.stream()
                                .filter(item -> item.getMaMh().equals(enrollCourseItem.getMaMh()))
                                .findAny();

                courseItem.ifPresent(itemLv -> listChoseCourse.remove(itemLv));

                enrollCourseItem.setStatusEnroll(StatusEnroll.DACHON);
                listChoseCourse.add(0, enrollCourseItem);
            }
            else{
                enrollCourseItem.setStatusEnroll(StatusEnroll.CHUACHON);
                listChoseCourse.remove(enrollCourseItem);
            }
        });
        return convertView;
    }
